...
| 阶段   | 项目              | 周期   | 目标                               | 状态        |
| ------ | ----------------- | ------ | ---------------------------------- | ----------- |
| 阶段 1 | learn-claude-code | 1-2 周 | 理解 Agent 原理 + Node.js 事件模型 | ✅ 已完成   |
| 阶段 2 | anything-llm      | 2-3 周 | 手撕 RAG + 后端基础件              | ✅ 已完成   |
| 阶段 3 | Flowise           | 2-3 周 | 架构设计 + 数据建模 + 工作流       | 🔄 进行中   |
| 阶段 4 | CopilotKit        | 1-2 周 | 前后端打通 + 作品集                | ⏳ 计划中   |

**当前聚焦**：Flowise — 本地部署、数据库 schema / ER 图、自定义 Node 与 API / 中间件链；anything-llm 相关笔记仍可作为 RAG / 流式参考：`../anything-llm/stream.js 聊天流式链路.md`。
