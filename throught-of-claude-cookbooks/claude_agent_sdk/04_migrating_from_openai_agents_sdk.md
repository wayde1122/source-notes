# 《Migrating from the OpenAI Agents SDK》学习笔记：会话管理与浏览

> 源 Notebook：`claude_agent_sdk/04_migrating_from_openai_agents_sdk.ipynb`
...
- @functiontool → @tool + createsdkmcpserver
- Agent(...) → ClaudeAgentOptions
- @inputguardrail / @outputguardrail → pre/post-call checks
...
### 5.2 核心函数 / 类定义：check_policy、has_dollar_amount、run_oai
...
这段代码对应 notebook 的第 6 个代码单元，核心关注点是 `check_policy、has_dollar_amount、run_oai`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
...
```
### 5.3 核心函数 / 类定义：check_policy_claude
...
这段代码对应 notebook 的第 9 个代码单元，核心关注点是 `check_policy_claude`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
### 5.4 核心函数 / 类定义：has_dollar_amount_hook
...
这段代码对应 notebook 的第 16 个代码单元，核心关注点是 `has_dollar_amount_hook`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
