# 《The one-liner research agent》学习笔记：Claude Agent SDK 工程实践

> 源 Notebook：`claude_agent_sdk/00_The_one_liner_research_agent.ipynb`
...
这段代码对应 notebook 的第 21 个代码单元，核心关注点是 `示例输入与运行入口`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
