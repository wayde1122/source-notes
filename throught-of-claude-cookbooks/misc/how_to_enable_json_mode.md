# 《Prompting Claude for "JSON mode》学习笔记：稳定 JSON 输出

> 源 Notebook：`misc/how_to_enable_json_mode.ipynb`

## 1. 学习目标

这篇 notebook 的学习目标是理解 **稳定 JSON 输出** 的核心机制：它如何准备输入、调用 Claude 或第三方服务、解析输出，并把示例流程迁移成自己的工程能力。
...
这篇 notebook 的核心价值在于：它用一个具体示例展示了 **稳定 JSON 输出** 如何从概念变成可运行、可验证、可迁移的 Claude 应用流程。
