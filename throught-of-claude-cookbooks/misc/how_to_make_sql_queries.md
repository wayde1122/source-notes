# 《How to make SQL queries with Claude》学习笔记：Claude API 工程技巧

> 源 Notebook：`misc/how_to_make_sql_queries.ipynb`
...
这段代码对应源 notebook 的第 8 个代码单元，重点关注 `ask_claude`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
### 5.3 主执行流程与 API 调用
