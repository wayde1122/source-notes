# 《Uploading" PDFs to Claude via the API》学习笔记：Claude API 工程技巧

> 源 Notebook：`misc/pdf_upload_summarization.ipynb`
...
这段代码对应源 notebook 的第 10 个代码单元，重点关注 `get_completion`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
