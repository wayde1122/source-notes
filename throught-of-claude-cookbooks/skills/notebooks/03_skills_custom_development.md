# 《Building custom Skills for Claude》学习笔记：Skills 能力封装

> 源 Notebook：`skills/notebooks/03_skills_custom_development.ipynb`
...
### 5.2 核心函数 / 类定义：create_skill、list_custom_skills、delete_skill
...
这段代码对应 notebook 的第 8 个代码单元，核心关注点是 `create_skill、list_custom_skills、delete_skill、test_skill`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
### 5.3 主执行流程与 API 调用
