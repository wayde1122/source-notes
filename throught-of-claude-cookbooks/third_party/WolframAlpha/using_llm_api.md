# 《Using the Wolfram Alpha LLM API as a tool with Claude》学习笔记：第三方生态集成

> 源 Notebook：`third_party/WolframAlpha/using_llm_api.ipynb`
...
### 5.2 核心函数 / 类定义：wolfram_alpha_query
...
这段代码对应源 notebook 的第 5 个代码单元，重点关注 `wolfram_alpha_query`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
### 5.3 核心函数 / 类定义：process_tool_call、chat_with_claude
...
这段代码对应源 notebook 的第 8 个代码单元，重点关注 `process_tool_call、chat_with_claude`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
### 5.4 主执行流程与 API 调用
