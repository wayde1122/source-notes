# 《Claude 3 RAG agents with LangChain v1》学习笔记：RAG 与外部知识集成

> 源 Notebook：`third_party/Pinecone/claude_3_rag_agent.ipynb`
...
这段代码对应源 notebook 的第 9 个代码单元，重点关注 `环境、依赖与数据准备`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
...
这段代码对应源 notebook 的第 24 个代码单元，重点关注 `主执行流程与 API 调用`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
...
这段代码对应源 notebook 的第 26 个代码单元，重点关注 `arxiv_search`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
