# 《Retrieval-augmented generation using Pinecone》学习笔记：评估体系构建

> 源 Notebook：`third_party/Pinecone/rag_using_pinecone.ipynb`
...
这段代码对应源 notebook 的第 6 个代码单元，重点关注 `环境、依赖与数据准备`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
### 5.2 主执行流程与 API 调用
...
这段代码对应源 notebook 的第 21 个代码单元，重点关注 `get_completion`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
### 5.4 核心函数 / 类定义：create_keyword_prompt
...
这段代码对应源 notebook 的第 22 个代码单元，重点关注 `create_keyword_prompt`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
