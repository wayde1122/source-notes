# 《ReAct agent》学习笔记：RAG 与外部知识集成

> 源 Notebook：`third_party/LlamaIndex/ReAct_Agent.ipynb`
...
这段代码对应源 notebook 的第 13 个代码单元，重点关注 `multiply、add`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
