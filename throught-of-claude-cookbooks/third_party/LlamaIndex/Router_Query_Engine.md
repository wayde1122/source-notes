# 《RouterQuery engine》学习笔记：RAG 与外部知识集成

> 源 Notebook：`third_party/LlamaIndex/Router_Query_Engine.ipynb`
...
这段代码对应源 notebook 的第 20 个代码单元，重点关注 `主执行流程与 API 调用`。阅读时要看清楚输入如何进入流程、Claude 或第三方服务在哪一步被调用、返回值如何被解析或展示。
