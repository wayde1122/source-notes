# 《Managed Agents tutorial: prompt versioning and rollback》学习笔记：会话管理与浏览

> 源 Notebook：`managed_agents/CMA_prompt_versioning_and_rollback.ipynb`
...
这段代码对应 notebook 的第 9 个代码单元，核心关注点是 `示例输入与运行入口`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
...
这段代码对应 notebook 的第 11 个代码单元，核心关注点是 `triage、score`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
