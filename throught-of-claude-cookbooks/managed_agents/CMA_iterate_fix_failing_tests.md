# 《Managed Agents tutorial: iterate on a failing test suite》学习笔记：会话管理与浏览

> 源 Notebook：`managed_agents/CMA_iterate_fix_failing_tests.ipynb`
...
这段代码对应 notebook 的第 4 个代码单元，核心关注点是 `主执行流程与 API 调用`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
### 5.3 主执行流程与 API 调用
