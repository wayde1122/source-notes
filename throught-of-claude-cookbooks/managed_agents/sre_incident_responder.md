# 《Build an SRE incident response agent with Claude Managed Agents》学习笔记：运维与事故响应 Agent

> 源 Notebook：`managed_agents/sre_incident_responder.ipynb`
...
这段代码对应 notebook 的第 9 个代码单元，核心关注点是 `upload`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
...
```
### 5.4 核心函数 / 类定义：handle_custom_tool、run_until_approval_or_end
...
这段代码对应 notebook 的第 13 个代码单元，核心关注点是 `handle_custom_tool、run_until_approval_or_end`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
