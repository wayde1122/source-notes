# 《Build a data analyst agent with Claude Managed Agents》学习笔记：数据分析 Agent

> 源 Notebook：`managed_agents/data_analyst_agent.ipynb`

## 1. 学习目标

这篇 notebook 的学习目标是理解 **数据分析 Agent** 的最小可运行路径：它如何准备输入、配置 Claude 或 Agent、连接工具 / 环境 / 外部资源，并把中间结果组织成可观察、可复用的工程流程。
...
这段代码对应 notebook 的第 7 个代码单元，核心关注点是 `主执行流程与 API 调用`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
### 5.3 主执行流程与 API 调用
...
这段代码对应 notebook 的第 11 个代码单元，核心关注点是 `主执行流程与 API 调用`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
...
这段代码对应 notebook 的第 13 个代码单元，核心关注点是 `wait_for_idle`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
...
这篇 notebook 的核心价值在于：它用一个具体示例展示了 **数据分析 Agent** 如何从概念变成可运行、可观察、可迁移的 Claude 工程流程。
