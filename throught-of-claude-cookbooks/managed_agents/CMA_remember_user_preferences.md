# 《Build agents that remember your users》学习笔记：Claude Managed Agents 托管式 Agent 实践

> 源 Notebook：`managed_agents/CMA_remember_user_preferences.ipynb`
...
这段代码对应 notebook 的第 7 个代码单元，核心关注点是 `run_turn`。阅读时要看清楚：输入从哪里来、Claude 或托管服务在哪一步被调用、返回结果如何进入后续流程。
### 5.3 主执行流程与 API 调用
